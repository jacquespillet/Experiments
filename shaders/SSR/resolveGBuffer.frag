...
void main()
{
    ...
    vec4 samplePositionRoughness = texture(positionTexture, fragUv);
    vec3 samplePosition = samplePositionRoughness.xyz;
    float roughness = samplePositionRoughness.w;
    ...
    vec3 ambientColor = vec3(ambient * diffuseColor);
    vec3 finalColor= ambientColor + Lo;
    
    vec4 ssr = texture(ssrTexture, fragUv);
    finalColor = mix(finalColor,ssr.rgb,(1.0f-roughness) *ssr.a);
    float gamma = 2.2;
