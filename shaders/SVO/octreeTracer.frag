...
vec3 GenRay() {
	vec2 coord = ivec2(gl_FragCoord.xy) / vec2(width, height);
	coord = coord * 2.0f - 1.0f;
	return normalize(mat3(inverse(viewMatrix)) * (inverse(projectionMatrix) * vec4(coord, 1, 1) ).xyz);
}

void main() {
	vec3 o = camPosition.xyz, d = GenRay();
