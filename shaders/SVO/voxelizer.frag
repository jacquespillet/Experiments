...
void main() {
	vec4 samp = texture(DiffuseTexture, gTexcoords);
	...
	//Store a list of voxels
	uint cur = atomicCounterIncrement(uCounter);
	//set fragment list
	if(uCountOnly == 0) {
		uvec3 uvoxel_pos = clamp(uvec3(gVoxelPos), uvec3(0u), uvec3(uVoxelResolution - 1u));
		
		uFragmentList[cur].x = uvoxel_pos.x | (uvoxel_pos.y << 12u) | ((uvoxel_pos.z & 0xffu) << 24u); // pack x, y and  last 8 bits of z
		uFragmentList[cur].y = ((uvoxel_pos.z >> 8u) << 28u) | ucolor.x | (ucolor.y << 8u) | (ucolor.z << 16u); //pack the the rest of z, r g and b.
	}
}
