...
void main()
{
    ...
    vec3 N = (vec4(texture(normalTexture, fragUv).xyz, 0.0f)).xyz;
    vec3 samplePosition = ( vec4(texture(positionTexture, fragUv).xyz, 1.0f)).xyz;
    vec3 AmbientOcclusion=vec3(1,1,1);
    if(ssaoEnabled>0) AmbientOcclusion = texture(ssaoTexture, fragUv).rgb;
    ...
    if(renderSSAO>0)
    {
        outputColor = vec4(AmbientOcclusion, 1.0f);
    }
    else
    {
        // vec3 ambientColor =vec3(0.1) * diffuseColor;
        vec3 ambientColor = vec3(diffuseColor) * 0.01f; // here we add occlusion factor
        // vec3 finalColor= (ambientColor + Lo) * AmbientOcclusion;
        vec3 finalColor= (ambientColor + Lo) * AmbientOcclusion;
