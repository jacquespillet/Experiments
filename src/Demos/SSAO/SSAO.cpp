...
void SSAO::InitGeometryBuffer()
{
    ...
    renderGBufferShader = GL_Shader("shaders/SSAO/renderGBuffer.vert", "", "shaders/SSAO/renderGBuffer.frag");    
    resolveGBufferShader = GL_Shader("shaders/SSAO/resolveGBuffer.vert", "", "shaders/SSAO/resolveGBuffer.frag"); 
...
void SSAO::InitSSAO()
{
    ...
    ssaoShader = GL_Shader("shaders/SSAO/SSAO.vert", "", "shaders/SSAO/SSAO.frag");
    blurShader = GL_Shader("shaders/SSAO/blur.vert", "", "shaders/SSAO/blur.frag");
...
void SSAO::RenderGUI() {
    ...
    ImGui::SliderInt("Power", &power, 1, 128);
    ImGui::SliderFloat("radius", &radius, 0, 4);
    ImGui::SliderFloat("bias", &bias, 0, 0.3f);
    ImGui::Checkbox("View SSAO", &renderSSAO);
...
void SSAO::Render() {
    ...
    
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(ssaoShader.programShaderObject, "projection"), 1, GL_FALSE, glm::value_ptr(cam.GetProjectionMatrix()));
        glUniformMatrix4fv(glGetUniformLocation(ssaoShader.programShaderObject, "projection"), 1, GL_FALSE, glm::value_ptr(cam.GetProjectionMatrix()));
        glUniform1i(glGetUniformLocation(ssaoShader.programShaderObject, "kernelSize"), kernelSize);
        glUniform1i(glGetUniformLocation(ssaoShader.programShaderObject, "power"), power);
        glUniform1f(glGetUniformLocation(ssaoShader.programShaderObject, "radius"), radius);
        glUniform1f(glGetUniformLocation(ssaoShader.programShaderObject, "bias"), bias);
    ...
    glUniform1f(glGetUniformLocation(resolveGBufferShader.programShaderObject, "ambient"), ambient);
...
void SSAO::MouseMove(float x, float y) {
    cam.mouseMoveEvent(x, y);
}

void SSAO::LeftClickDown() {
    cam.mousePressEvent(0);
}

void SSAO::LeftClickUp() {
    cam.mouseReleaseEvent(0);
}

void SSAO::RightClickDown() {
    cam.mousePressEvent(1);
}

void SSAO::RightClickUp() {
    cam.mouseReleaseEvent(1);
}

void SSAO::Scroll(float offset) {
    cam.Scroll(offset);
}
