...
void Boids::InitBuffers()
{
    CreateComputeShader("shaders/Boids/boids.compute", &nbodiesShader);	
    pointsShader = GL_Shader("shaders/Boids/points.vert", "shaders/Boids/points.geom", "shaders/Boids/points.frag");
...
void Boids::MouseMove(float x, float y) {
    cam.mouseMoveEvent(x, y);
}

void Boids::LeftClickDown() {
    cam.mousePressEvent(0);
	UpdateParticles();
}

void Boids::LeftClickUp() {
    cam.mouseReleaseEvent(0);
}

void Boids::RightClickDown() {
    cam.mousePressEvent(1);
	UpdateParticles();
}

void Boids::RightClickUp() {
    cam.mouseReleaseEvent(1);
}

void Boids::Scroll(float offset) {
    cam.Scroll(offset);
}
